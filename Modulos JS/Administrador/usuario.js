...
var Inventario = new Vue({
    ...
    methods: {
        ...
        onFileChange: function (event) {
            this.selectedFile = event.target.files[0]; //se guarda el archivo subido en una variable
            /* Si se sube una imagen y tiene el formato correcto se llama al método para la actualización de la ruta en la base de datos */
            if (this.selectedFile && (this.selectedFile.type === 'image/jpeg' || this.selectedFile.type === 'image/png')) {
                this.imageUrl = URL.createObjectURL(this.selectedFile);
                this.uploadImage(); //llama al método para actualizar la ruta de la imagen en la base de datos
            } else {
                // Si ni es una imagen en el formato correcto se mostrará un mensaje de error
                Swal.fire({
                    title: 'Error!',
